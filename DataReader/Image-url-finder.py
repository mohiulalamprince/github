...
class Reader:
    ...
    def __init__(self, inDirectory, blockSize= 1024*1024*200):
        ...
        self.default_dir = "H:\\image_url_nelly\\"
    ...
    def readHostData(self, hostURLsInfo):
        ...
        for urlInfo in hostURLsInfo:
            ...
            output_string = ""
            output_string += "[BYTE_INFO:]" + str(urlInfo.contentNo) + " " + str(urlInfo.urlStartByte) + " " + str(urlInfo.urlEndByte) + " " + str(urlInfo.contentStartByte) +  " " + str(urlInfo.contentEndByte) 

            strURL, strContent = self.getURLAndContent(urlInfo)
            
            output_string += "[URL:]" + str(strURL)
            
            self.processData(output_string, strURL, strContent)

    def processData(self, output_string, thisURL, thisContent):
        ...
        if (self.enable_downloader == True):            
            ...
            print "URL : " + thisURL
            self.url_image_file_fp = open(self.default_dir + "image_url_list.txt", "a+b")
            
            for img in imgs:
                self.image_counter += 1
                img_url = img["src"]
                self.url_image_file_fp.write(str(output_string) + "[IMAGE_URL:]"+ str(img_url) + "[IMAGE_ID:]" + str(self.image_counter) + "\n")
                
            self.url_image_file_fp.close()

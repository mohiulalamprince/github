...
class Reader:
    ...
    def __init__(self, inputDirectory, outputDirectoy, readSZ = 32):
        ...
        self.indexSpliter = IndexSpliter(self.inputDirectory, self.outputDirectoy)
    ...
    def memoryInfo(self, memSZ):
        
        strRes = str(memSZ/1024) + " GB "
        strRes += str(memSZ%1024) + " MB"
        strRes = (13 - len(strRes)) * " " + strRes
        
        return strRes
    
    def timeInfo(self, strTime):
        
        strRes = str(int(strTime/60)) + " HR "
        strRes += str(int(strTime%60)) + " MINUTE(s)"
        strRes = (20 - len(strRes)) * " " + strRes
    ...
    def printStatus(self):
        ...
        self.printLog("Total Content Size             : " + ("%s")%self.memoryInfo(self.totalContentSize))
        self.printLog("Total Data Processed           : " + ("%s")%self.memoryInfo(self.totalDataProcessed/(1024*1024)))
        self.printLog("Data Remaining                 : " + ("%s")%self.memoryInfo(dataLeft))
        self.printLog("Total Running Time             : " + ("%s")%(self.timeInfo(self.totalRuningTime)))              
        self.printLog("Time Remaining                 : " + ("%s")%(self.timeInfo(tLeft)))
        self.printLog("Total Documents Processed      : " + ("%10ld")%(self.totalDocs) + " DOCUMENT(s)")
        self.printLog("Total Documents in Last Block  : " + ("%10ld")%(self.totalURLsInLastBlock) + " DOCUMENT(s)")
        self.printLog("Process Time for Last Block    : " + ("%10.4lf")%pTime + " MINUTE(s)")
    ...
    def loadContent(self, seekPosition):
        ...
        #self.printLog("Process Time for Last Block    : " + ("%.4lf")%pTime + " Sec(s)")
    ...
    def read(self, startByte, endByte):
        ...
        if endByte > self.lastContentSeek + len(self.data):
    ...
    def readData(self, strURLInfo):
        ...
        if urlInfo.status == "start":
            ...
            self.processData(urlInfo.hostName, strURL, strContent, urlInfo)
    ...
    def processContentIndexBlock(self, indexNo):
        ...
        if not os.path.exists(contentFileName):
            print "Content File Not Found: ", contentFileName
            return
